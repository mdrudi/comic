...
def bm_setup() :
   ...
   t_compute=0
   t_write=0
   bminfo=dict(t_bm=0,t_wrap=0,in_byte=0,in_point=0,in_spoint=0,out_byte=0,out_point=0,out_spoint=0,p_max_mem=0)
...
def bm_close() :
   ...
   print '\nInit         : (ms)',int(t_init),'-',int(round(t_init/tot100)),'%'
   print 'i/o read     : (ms)',int(t_read),'-',int(round(t_read/tot100)),'%'
   print 'computation  : (ms)',int(t_compute),'-',int(round(t_compute/tot100)),'%'
   print 'i/o write    : (ms)',int(t_write),'-',int(round(t_write/tot100)),'%'
